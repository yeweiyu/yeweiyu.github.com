...
+function ($) { "use strict";
    ...
    var Request = function (element, handler, options) {
        ...
        var ajaxUrlPos=handler.indexOf('/'),action;
        if(ajaxUrlPos!=-1){
            options.url = handler.substr(ajaxUrlPos).replace(/^\/(https?:\/\/)/,'$1');
            handler=handler.substr(0,ajaxUrlPos);
        }else{
            var form=element.parents('form'),action=form.attr('action');
            if(form.data('requestUseAction')&&action){
                options.url=action;
            }
        }
        /*
        ...
        var requestOptions = {
            ...
            handleUpdateResponse: function(data, textStatus, jqXHR) {
                var updateElelent=function (selector,partial,afterhandler) {
                    var el;
                    if (jQuery.type(selector) == 'string') {
                        switch (selector.charAt(0)) {
                            ...
                            case '=':
                                ...
                                break;
                            case '~':
                                var res = data[partial];
                                if (jQuery.type(res) == 'string' && res.substr(0, 10) == "(function(") {
                                    data[partial]=eval(res).call(data);
                                }
                                el=$(selector.substring(1));
                                if(el.attr('type')=='text/html'&&el[0].nodeName=='SCRIPT'){
                                    data[partial]=template.compile(el.html())(data[partial]);
                                ...
                                var handler=el.attr('data-ajax-after');
                                updateElelent(selector,partial,handler?new Function(handler):'');
                                return;
                            default:
                                el=$(selector);
                                el.trigger('ajaxBeforeReplace');
                                el.html(data[partial]);
                                break;
                        }
                    } else {
                        el=$(selector);
                    ...
                    el.trigger('ajaxUpdate', [context, data, textStatus, jqXHR]);
                    if($.isFunction(afterhandler)){
                        afterhandler.call(el);
                    }else{
                        (new Function(el.attr('data-ajax-after'))).call(el)
                    }
                };
                /*
                ...
                var updatePromise = $.Deferred().done(function() {
                    for (var partial in data) {
                         ...
                         */
                        updateElelent((options.update[partial]) ? options.update[partial] : partial,partial);
                    }
